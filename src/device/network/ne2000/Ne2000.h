...
protected:
  /** Overwrite NetworkDevice handleOutgoingPacket() */
...
private:
  ...
  void sendPacketToNIC(const uint8_t *packet, uint32_t length);
  ...
  uint32_t sendStartPage = 0x40;
  uint8_t PSTART = 0x46;
  uint8_t PSTOP = 0x60;
  uint8_t nextPacket = PSTART + 1;
